...
export function Setup(props: inferSSRProps<typeof getServerSideProps>) {
    ...
    const steps: React.ComponentProps<typeof WizardForm>["steps"] = [
        {
            ...
            content: (setIsLoading) => (<AdminUser onSubmit={() => {
                    setIsLoading(true);
                }} onSuccess={() => {
                    setStep(2);
                }} onError={() => {
        ...
        },
        {
            ...
            content: (setIsLoading) => {
                return (<ChooseLicense id="wizard-step-2" name="wizard-step-2" value={value} onChange={setValue} onSubmit={() => {
                        setIsLoading(true);
                        setStep(3);
                    }}/>);
            },
    ...
    if (!isFreeLicense) {
        steps.push({
            ...
            content: (setIsLoading) => {
                const currentStep = 3;
                return (<EnterpriseLicense id={`wizard-step-${currentStep}`} name={`wizard-step-${currentStep}`} onSubmit={() => {
                        setIsLoading(true);
                    }} onSuccess={() => {
    ...
    steps.push({
        ...
        content: (setIsLoading) => {
            ...
            return (<AdminAppsList id={`wizard-step-${currentStep}`} name={`wizard-step-${currentStep}`} classNames={{
                ...
                }} baseURL={`/auth/setup?step=${currentStep}`} useQueryParam={true} onSubmit={() => {
                    setIsLoading(true);
                    router.replace("/");
                }}/>);
        },
    ...
    return (<>
      ...
      <main className="bg-subtle flex items-center print:h-full md:h-screen">
        <WizardForm href="/auth/setup" steps={steps} nextLabel={t("next_step_text")} finishLabel={t("finish")} prevLabel={t("prev_step")} stepLabel={(currentStep, maxSteps) => t("current_step_of_total", { currentStep, maxSteps })}/>
      </main>
    </>);
}
