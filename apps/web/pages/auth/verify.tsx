...
export default function Verify() {
    ...
    const { t, sessionId, stripeCustomerId } = querySchema.parse(...Object.fromEntries(searchParams ?? new URLSearchParams()));
    ...
    return (<div className="text-inverted bg-black bg-opacity-90 backdrop-blur-md backdrop-grayscale backdrop-filter">
      <Head>
        <title>
      ...
      <div className="flex min-h-screen flex-col items-center justify-center px-6">
        <div className="m-10 flex max-w-2xl flex-col items-start border border-white p-12 text-left">
          <div className="rounded-full border border-white p-3">
            {hasPaymentFailed ? (<ExclamationIcon className="text-inverted h-12 w-12 flex-shrink-0 p-0.5 font-extralight"/>) : sessionId ? (<CheckIcon className="text-inverted h-12 w-12 flex-shrink-0 p-0.5 font-extralight"/>) : (<MailOpenIcon className="text-inverted h-12 w-12 flex-shrink-0 p-0.5 font-extralight"/>)}
          </div>
          ...
          {hasPaymentFailed && (<p className="my-6">Your account has been created, but your premium has not been reserved.</p>)}
          <p>
          ...
          <div className="mt-6 flex space-x-5 text-center">
            <Button color="secondary" disabled={secondsLeft > 0} onClick={async (e) => {
            if (!customer) {
                return;
            }
            e.preventDefault();
