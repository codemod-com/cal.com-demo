...
export default function Apps({ apps }: InferGetStaticPropsType<typeof getStaticProps>) {
    ...
    return (<>
      <Shell isPublic backPath="/apps" smallHeading heading={<>
            <Link href="/apps" className="text-emphasis inline-flex items-center justify-start gap-1 rounded-sm py-2">
              {isLocaleReady ? t("app_store") : <SkeletonText className="h-4 w-24"/>}{" "}
            </Link>
            {category && (<span className="text-default gap-1">
                <span>&nbsp;/&nbsp;</span>
