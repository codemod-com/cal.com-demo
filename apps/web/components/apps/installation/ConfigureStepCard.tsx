      ...
      <Form
        ...
        <div>
          <div className="sm:border-subtle bg-default relative border p-4 dark:bg-black sm:rounded-md">
            ...
            />
            <button type="submit" className="hidden" form={`eventtype-${eventType.id}`} ref={ref}>{t('save')}</button>
          </div>
        </div>
      </Form>
    );
...
return (
        ...
        <button form="outer-event-type-form" type="submit" className="hidden" ref={mainForSubmitRef}>{t('save-duplicate')}</button>
        <Button
