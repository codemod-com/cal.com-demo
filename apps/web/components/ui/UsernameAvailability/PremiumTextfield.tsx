...
const { t } = useTranslation();
  ...
  let paymentMsg = !currentUsername ? (
    <span className="text-xs text-orange-400">{t('reserve-premium-username')}{getPremiumPlanPriceValue()}
    </span>
  ) : null;

  if (recentAttemptPaymentStatus && recentAttemptPaymentStatus !== "paid") {
    paymentMsg = (
      <span className="text-sm text-red-500">{t('payment-not-completed-username-not-reserved')}</span>
    );
  }
