...
const { t } = useTranslation("../components/settings/platform/oauth-clients");
...
/>
        ...
        <div className="border-subtle flex text-sm">
          ...
          {permissions ? <div className="flex">{clientPermissions}</div> : <>{t('disabled-text')}</>}
        </div>
      ...
      <div className="flex items-start gap-4">
        <Button
          ...
          loading={isLoading}
          disabled={isLoading}
          onClick={() => router.push(`/settings/platform/oauth-clients/${id}/edit/webhooks`)}>{t('webhooks-title')}</Button>
        <Button
          color="secondary"
          loading={isLoading}
          disabled={isLoading}
