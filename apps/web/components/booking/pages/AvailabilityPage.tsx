...
import { useEmbedNonStylesConfig, useEmbedStyles, useEmbedUiConfig, useIsBackgroundTransparent, useIsEmbed, } from "@calcom/embed-core/embed-iframe";
...
const AvailabilityPage = ({ profile, eventType, ...restProps }: Props) => {
    ...
    const timezoneDropdown = useMemo(() => <TimezoneDropdown timeZone={timeZone} onChangeTimeZone={setTimeZone}/>, [timeZone]);
    const paymentAppData = getPaymentAppData(eventType);
    
    const rainbowAppData = getEventTypeAppData(eventType, "rainbow") || {};
    const rawSlug = profile.slug ? profile.slug.split("/") : [];
    if (rawSlug.length > 1)
        rawSlug.pop(); //team events have team name as slug, but user events have [user]/[type] as slug.
    ...
    return (<Gates gates={gates} appData={rainbowAppData} dispatch={gateDispatcher}>
      <HeadSeo title={`${rescheduleUid ? t("reschedule") : ""} ${eventType.title} | ${profile.name}`} description={`${rescheduleUid ? t("reschedule") : ""} ${eventType.title}`} meeting={{
      ...
      <div>
        <main className={classNames("flex flex-col md:mx-4", shouldAlignCentrally ? "items-center" : "items-start", !isEmbed && classNames("bg-subtle dark:bg-default mx-auto my-0 ease-in-out md:my-24"))}>
          <div>
            <div style={availabilityDatePickerEmbedStyles} className={classNames(isBackgroundTransparent ? "" : "bg-default dark:bg-muted pb-4 md:pb-0", "border-booker md:border-booker-width md:rounded-md", isEmbed && "mx-auto")}>
              <div className="md:flex">
                {showEventTypeDetails && (<div className={classNames(" border-subtle flex flex-col p-5 sm:border-r", "min-w-full md:w-[230px] md:min-w-[230px]", recurringEventCount && "xl:w-[380px] xl:min-w-[380px]")}>
                    <BookingDescription profile={profile} eventType={eventType} rescheduleUid={rescheduleUid}>
                      {rescheduleUid && eventType.seatsPerTimeSlot && bookingAttendees && (<div className={classNames("flex flex-nowrap items-center text-sm font-medium", " text-default", "ltr:mr-[10px] rtl:ml-[10px]")}>
                          <User className={classNames("min-h-4 min-w-4 ml-[2px] inline-block ltr:mr-[10px] rtl:ml-[10px]", "mt-[2px]")}/>{" "}
                          {t("event_type_seats", { numberOfSeats: bookingAttendees })}
                        </div>)}
                      {!rescheduleUid && eventType.recurringEvent && (<div className="flex items-start text-sm font-medium">
                          ...
                          <div>
                            ...
                            <NumberInput defaultValue={eventType.recurringEvent.count} min="1" max={eventType.recurringEvent.count} isFullWidth={false} className="me-2 inline w-16" onChange={(event) => {
                    ...
                    count: recurringEventCount,
                      ...
                      {paymentAppData.price > 0 && (<p className="-ml-2 px-2 text-sm font-medium">
            ...
            <div>
              ...
              <p className="text-subtle line-through ">
                ...
                <SlotPicker weekStart={typeof profile.weekStart === "string"
            ? ([
                "Sunday",
                "Monday",
                "Tuesday",
            ...
            : profile.weekStart /* Allows providing weekStart as number */} eventType={eventType} timeFormat={timeFormat} onTimeFormatChange={onTimeFormatChange} timeZone={timeZone} users={userList} seatsPerTimeSlot={eventType.seatsPerTimeSlot || undefined} bookingAttendees={bookingAttendees || undefined} recurringEventCount={recurringEventCount} ethSignature={gateState.rainbowToken}/>
              </div>
            </div>
