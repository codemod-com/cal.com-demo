...
const AvailableTimes: FC<AvailableTimesProps> = ({ slots = [], isLoading, date, eventTypeId, eventTypeSlug, recurringCount, timeFormat, onTimeFormatChange, seatsPerTimeSlot, bookingAttendees, ethSignature, duration, }) => {
    ...
    return (<div ref={slotPickerRef}>
      {!!date ? (<div className="mt-8 flex h-full w-full flex-col rounded-md px-4 text-center sm:mt-0 sm:p-4 md:-mb-5 md:min-w-[200px] md:p-4 lg:min-w-[300px]">
          <div className="mb-4 flex items-center text-left text-base">
            ...
            <div className="ml-auto">
              <ToggleGroup onValueChange={(timeFormat) => onTimeFormatChange(timeFormat === "24")} defaultValue={timeFormat === TimeFormat.TWELVE_HOUR ? "12" : "24"} options={[
                { value: "12", label: t("12_hour_short") },
                { value: "24", label: t("24_hour_short") },
            ]}/>
            </div>
          </div>
          <div ref={ref} className="scroll-bar scrollbar-track-w-20 relative -mb-4 flex-grow overflow-y-auto sm:block md:h-[364px]">
            {slots.length > 0 &&
                slots.map((slot) => {
                    type BookingURL = {
                        pathname: string;
                    ...
                    return (<div data-slot-owner={(slot.userIds || []).join(",")} key={`${dayjs(slot.time).format()}`}>
                    ...
                    {seatsPerTimeSlot && slot.attendees && (slotFull || notEnoughSeats) ? (<div className={classNames("text-default bg-default border-subtle mb-2 block rounded-sm border py-2 font-medium opacity-25", brand === "#fff" || brand === "#ffffff" ? "" : "")}>
                        {dayjs(slot.time).tz(timeZone()).format(timeFormat)}
                        {notEnoughSeats ? (<p className="text-sm">{t("not_enough_seats")}</p>) : slots ? (<p className="text-sm">{t("booking_full")}</p>) : null}
                      </div>) : (<Link href={bookingUrl} prefetch={false} className={classNames(" bg-default dark:bg-muted border-default hover:bg-subtle hover:border-brand-default text-emphasis mb-2 block rounded-md border py-2 text-sm font-medium", brand === "#fff" || brand === "#ffffff" ? "" : "")} onClick={() => reserveSlot(slot)} data-testid="time">
                        {dayjs(slot.time).tz(timeZone()).format(timeFormat)}
                        {!!seatsPerTimeSlot && (<p className={`${slot.attendees && slot.attendees / seatsPerTimeSlot >= 0.8
                                    ? "text-rose-600"
            ...
            {!isLoading && !slots.length && (<div className="-mt-4 flex h-full w-full flex-col content-center items-center justify-center">
