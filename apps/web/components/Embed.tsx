...
import { Button, Dialog, DialogClose, DialogContent, HorizontalTabs, Label, showToast, Switch, TextArea, TextField, ColorPicker, Select, } from "@calcom/ui";
...
const getInstructionString = ({ apiName, instructionName, instructionArg, }: {
    apiName: string;
...
const Codes: Record<string, Record<string, (...args: any[]) => string>> = {
    react: {
        inline: ({ calLink, uiInstructionCode, previewState, }: {
            calLink: string;
...
function MyComponent() {
  ...
  return <Cal calLink="${calLink}" style={{width:"${width}",height:"${height}",overflow:"scroll"}} />;
        ...
        "floating-popup": ({ floatingButtonArg, uiInstructionCode, }: {
  ...
  useEffect(()=>{
    (async function () {
      ...
      ${uiInstructionCode}
        ...
        "element-click": ({ calLink, uiInstructionCode }: {
            calLink: string;
            uiInstructionCode: string;
        }) => {
            return code `
...
function MyComponent() {
  ...
  return <button data-cal-link="${calLink}" />;
    ...
    HTML: {
        inline: ({ calLink, uiInstructionCode }: {
            calLink: string;
            uiInstructionCode: string;
        }) => {
...
const getEmbedTypeSpecificString = ({ embedFramework, embedType, calLink, previewState, }: {
    ...
    return [
        {
            ...
            type: "inline",
            illustration: (<svg width="100%" height="100%" className="rounded-md" viewBox="0 0 308 265" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M0 1.99999C0 0.895423 0.895431 0 2 0H306C307.105 0 308 0.895431 308 2V263C308 264.105 307.105 265 306 265H2C0.895431 265 0 264.105 0 263V1.99999Z" fill="white"/>
...
const tabs = [
    {
        ...
        icon: Code,
        type: "code",
        Component: forwardRef<HTMLTextAreaElement | HTMLIFrameElement | null, {
            embedType: EmbedType;
            calLink: string;
            previewState: PreviewState;
          ...
          <TextArea data-testid="embed-code" ref={ref as typeof ref & MutableRefObject<HTMLTextAreaElement>} name="embed-code" className="text-default bg-default selection:bg-subtle h-[calc(100%-50px)] font-mono" style={{ resize: "none", overflow: "auto" }} readOnly value={`<!-- Cal ${embedType} embed code begins -->\n` +
                    (embedType === "inline"
                        ? `<div style="width:${getDimension(previewState.inline.width)};height:${getDimension(previewState.inline.height)};overflow:scroll" id="my-cal-inline"></div>\n`
                        : "") +
...
<!-- Cal ${embedType} embed code ends -->`}/>
    ...
    },
    {
        ...
        Component: forwardRef<HTMLTextAreaElement | HTMLIFrameElement | null, {
          ...
          <TextArea data-testid="embed-react" ref={ref as typeof ref & MutableRefObject<HTMLTextAreaElement>} name="embed-react" className="text-default bg-default selection:bg-subtle h-[calc(100%-50px)] font-mono" readOnly style={{ resize: "none", overflow: "auto" }} value={`/* First make sure that you have installed the package */
...
`}/>
    ...
    },
    {
        ...
        icon: Trello,
        type: "iframe",
        Component: forwardRef<HTMLIFrameElement | HTMLTextAreaElement | null, {
            calLink: string;
            embedType: EmbedType;
            previewState: PreviewState;
            ...
            return (<iframe ref={ref as typeof ref & MutableRefObject<HTMLIFrameElement>} data-testid="embed-preview" className="h-[100vh] border" width="100%" height="100%" src={`${WEBAPP_URL}/embed/preview.html?embedType=${embedType}&calLink=${calLink}`}/>);
        }),
    },
];
...
const ChooseEmbedTypesDialogContent = () => {
    ...
    return (<DialogContent className="rounded-lg p-10" type="creation" size="lg">
      <div className="mb-2">
      ...
      <div className="items-start space-y-2 md:flex md:space-y-0">
        {embeds(t).map((embed, index) => (<button className="hover:bg-subtle bg-muted w-full rounded-md border border-transparent p-6 text-left hover:rounded-md ltr:mr-4 ltr:last:mr-0 rtl:ml-4 rtl:last:ml-0 lg:w-1/3" key={index} data-testid={embed.type} onClick={() => {
                goto(router, {
...
const EmbedTypeCodeAndPreviewDialogContent = ({ embedType, embedUrl, }: {
    ...
    const inlineEmbedDimensionUpdate = ({ width, height }: {
        width: string;
        height: string;
    }) => {
        iframeRef.current?.contentWindow?.postMessage({
    ...
    return (<DialogContent ref={dialogContentRef} className="rounded-lg p-0.5 sm:max-w-[80rem]" enableOverflow type="creation">
      <div className="flex">
        <div className="bg-muted flex w-1/3 flex-col p-8">
          <h3 className="text-emphasis mb-2.5 flex items-center text-xl font-semibold leading-5" id="modal-title">
          ...
          <div className="flex flex-col">
            <div className={classNames("font-medium", embedType === "element-click" ? "hidden" : "")}>
              <Collapsible open={isEmbedCustomizationOpen} onOpenChange={() => setIsEmbedCustomizationOpen((val) => !val)}>
                  ...
                  <div className={classNames(embedType === "inline" ? "block" : "hidden")}>
                    ...
                    <div className="justify-left mb-6 flex items-center !font-normal ">
                      <div className="mr-[9px]">
                        <TextField labelProps={{ className: "hidden" }} className="focus:ring-offset-0" required value={previewState.inline.width} onChange={(e) => {
            setPreviewState((previewState) => {
                ...
                return {
                    ...
                    inline: {
                      ...
                      <TextField labelProps={{ className: "hidden" }} className="focus:ring-offset-0" value={previewState.inline.height} required onChange={(e) => {
            ...
            setPreviewState((previewState) => {
                return {
                  ...
                  <div className={classNames("items-center justify-between", embedType === "floating-popup" ? "text-emphasis" : "hidden")}>
                    <div className="mb-2 text-sm">Button text</div>
                    {/* Default Values should come from preview iframe */}
                    <TextField labelProps={{ className: "hidden" }} onChange={(e) => {
            setPreviewState((previewState) => {
                return {
                    ...previewState,
                    floatingPopup: {
        ...
        }} defaultValue={t("book_my_cal")} required/>
                  </div>
                  <div className={classNames("mt-4 flex items-center justify-start", embedType === "floating-popup"
            ...
            setPreviewState((previewState) => {
                return {
                  ...
                  <div className={classNames("mt-4 items-center justify-between", embedType === "floating-popup" ? "text-emphasis" : "hidden")}>
                    <div className="mb-2">Position of button</div>
                    <Select onChange={(position) => {
            setPreviewState((previewState) => {
                return {
                    ...previewState,
                    floatingPopup: {
        ...
        }} defaultValue={FloatingPopupPositionOptions[0]} options={FloatingPopupPositionOptions}/>
                  </div>
                  <div className="mt-3 flex flex-col xl:flex-row xl:justify-between">
                    <div className={classNames("mt-4", embedType === "floating-popup" ? "" : "hidden")}>
                      ...
                      <div className="mt-2 w-40 xl:mt-0 xl:w-full">
                        <ColorPicker className="w-[130px]" popoverAlign="start" container={dialogContentRef?.current ?? undefined} defaultValue="#000000" onChange={(color) => {
            setPreviewState((previewState) => {
                return {
                    ...previewState,
                    floatingPopup: {
            ...
            <div className="font-medium">
              <Collapsible open={isBookingCustomizationOpen} onOpenChange={() => setIsBookingCustomizationOpen((val) => !val)}>
                <CollapsibleContent>
                  <div className="text-sm">
                    <Label className="mb-6">
                      <div className="mb-2">Theme</div>
                      <Select className="w-full" defaultValue={ThemeOptions[0]} components={{
            ...
            setPreviewState((previewState) => {
                return {
                    ...
                    <div className="mb-6 flex items-center justify-start space-x-2 rtl:space-x-reverse">
                      <Switch checked={previewState.hideEventTypeDetails} onCheckedChange={(checked) => {
            setPreviewState((previewState) => {
                return {
                    ...previewState,
        ...
        ].map((palette) => (<Label key={palette.name} className="mb-6">
                        <div className="mb-2">{palette.title}</div>
                        <div className="w-full">
                          <ColorPicker popoverAlign="start" container={dialogContentRef?.current ?? undefined} defaultValue="#000000" onChange={(color) => {
                addToPalette({
        ...
        <div className="flex w-2/3 flex-col p-8">
          ...
          {tabs.map((tab) => {
            return (<div key={tab.href} className={classNames(searchParams?.get("embedTabName") === tab.href.split("=")[1] ? "flex flex-grow flex-col" : "hidden")}>
                <div className="flex h-[55vh] flex-grow flex-col">
                  {tab.type === "code" ? (<tab.Component embedType={embedType} calLink={calLink} previewState={previewState} ref={refOfEmbedCodesRefs.current[tab.name]}/>) : (<tab.Component embedType={embedType} calLink={calLink} previewState={previewState} ref={iframeRef}/>)}
                </div>
                <div className={searchParams?.get("embedTabName") == "embed-preview" ? "block" : "hidden"}/>
                <div className="mt-8 flex flex-row-reverse gap-x-2">
                  {tab.type === "code" ? (<Button type="submit" onClick={() => {
...
export const EmbedDialog = () => {
    ...
    return (<Dialog name="embed" clearQueryParamsOnClose={queryParamsForDialog} onOpenChange={(open) => {
            if (!open)
                window.resetEmbedStatus();
        }}>
      {!searchParams?.get("embedType") ? (<ChooseEmbedTypesDialogContent />) : (<EmbedTypeCodeAndPreviewDialogContent embedType={searchParams?.get("embedType") as EmbedType} embedUrl={embedUrl}/>)}
    </Dialog>);
};
...
export const EmbedButton = <T extends React.ElementType>({ embedUrl, children, className = "", as, ...props }: EmbedButtonProps<T> & React.ComponentPropsWithoutRef<T>) => {
    const router = useRouter();
    ...
    return (<Component {...props} className={className} data-test-embed-url={embedUrl} data-testid="embed" type="button" onClick={() => {
            openEmbedModal();
        }}>
      {children}
    </Component>);
