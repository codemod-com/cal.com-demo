...
export default function useRouterQuery<T extends string>(name: T) {
    ...
    const setQuery = (newValue: string | number | null | undefined) => {
        router.replace({ query: { ...Object.fromEntries(searchParams ?? new URLSearchParams()), [name]: newValue } }, undefined, { shallow: true });
        router.replace({ query: { ...Object.fromEntries(searchParams ?? new URLSearchParams()), ...query, [name]: newValue } }, undefined, { shallow: true });
    };
