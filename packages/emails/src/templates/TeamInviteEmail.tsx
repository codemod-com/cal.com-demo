...
/></Trans>{" "}
                <Trans
                  ...
                  <br /><Trans
i18nKey="personal-event-types-moved-organization"
values={{ _teamName_: <>{teamName}</> }}
components={{"0": 
                  <strong />}}
/><br />
                  <br />{t('recommend-creating-new-account-2')}</Trans>
              </>
            )}
          </>
        );
      ...
      return (
        <>
          {autoJoinType === "added" ? (
            <Trans i18nKey="email_team_invite|content|added_to_org"><Trans
i18nKey="invited-by-added-to-team-organization-2"
values={{ invitedBy, _teamName_: <>{teamName}</> }}
components={{"0": <strong />}}
/></Trans>
          ) : (
...
/></Trans>
          ...
          <Trans>{t('app-name-event-juggling-scheduler-2', { appName })}</Trans>
        </>
      );
    }
