...
export const UserForm = ({ defaultValues, localeProp = "en", onSubmit = noop, submitLabel = "save", }: {
    ...
    const form = useForm<FormValues>({
        defaultValues: {
            ...
            role: {
                ...
                label: userRoleOptions.find((option) => option.value === defaultValues?.role)?.label ||
                    userRoleOptions[0].label,
            },
            identityProvider: {
                value: defaultValues?.identityProvider || identityProviderOptions[0].value,
                label: identityProviderOptions.find((option) => option.value === defaultValues?.role)?.label ||
                    identityProviderOptions[0].label,
            },
        },
    });
    
    return (<Form form={form} className="space-y-4" handleSubmit={onSubmit}>
      ...
      <Controller name="role" control={form.control} render={({ field: { onChange, value } }) => (<div>
      ...
      <Controller name="identityProvider" control={form.control} render={({ field: { onChange, value } }) => (<div>
      ...
      <Controller name="locale" render={({ field: { value, onChange } }) => (<>
            ...
            label: string;
            value: string;
        }> className="capitalize" options={localeOptions} value={value} onChange={onChange}/>
          </>)}/>
      <Controller name="timeZone" control={form.control} render={({ field: { value } }) => (<>
            <Label className="text-default mt-8">
              <>{t("timezone")}</>
            </Label>
            <TimezoneSelect id="timezone" value={value} onChange={(event) => {
                if (event)
                    form.setValue("timeZone", event.value);
            }}/>
          </>)}/>
      <Controller name="timeFormat" control={form.control} render={({ field: { value } }) => (<>
            <Label className="text-default mt-8">
              <>{t("time_format")}</>
            </Label>
            <Select value={value} options={timeFormatOptions} onChange={(event) => {
                if (event)
                    form.setValue("timeFormat", { ...event });
            }}/>
          </>)}/>
      <Controller name="weekStart" control={form.control} render={({ field: { value } }) => (<>
            <Label className="text-default mt-8">
              <>{t("start_of_week")}</>
            </Label>
            <Select value={value} options={weekStartOptions} onChange={(event) => {
                if (event)
                    form.setValue("weekStart", { ...event });
            }}/>
          </>)}/>
