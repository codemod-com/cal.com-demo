...
function WorkflowPage() {
    ...
    const { workflow: workflowId } = true ? querySchema.parse(...Object.fromEntries(searchParams ?? new URLSearchParams())) : { workflow: -1 };
    const utils = trpc.useContext();
    
    const { data: workflow, isError, error, isLoading, } = trpc.viewer.workflows.get.useQuery({ id: +workflowId }, {
        enabled: true && !!workflowId,
    });
    
    const { data: verifiedNumbers } = trpc.viewer.workflows.getVerifiedNumbers.useQuery({ teamId: workflow?.team?.id }, {
        enabled: !!workflow?.id,
    });
    
    const readOnly = workflow?.team?.members?.find((member) => member.userId === session.data?.user.id)?.role ===
    ...
    useEffect(() => {
        if (workflow && !isLoading) {
            ...
            setSelectedEventTypes(workflow.activeOn.flatMap((active) => {
                if (workflow.teamId && active.eventType.parentId)
                    return [];
                return {
    ...
    const updateMutation = trpc.viewer.workflows.update.useMutation({
        onSuccess: async ({ workflow }) => {
            if (workflow) {
                ...
                showToast(t("workflow_updated_successfully", {
    ...
    return session.data ? (<Form form={form} handleSubmit={async (values) => {
      ...
      <Shell backPath="/workflows" title={workflow && workflow.name ? workflow.name : "Untitled"} CTA={!readOnly && (<div>
              <Button type="submit">{t("save")}</Button>
            </div>)} hideHeadingOnMobile heading={session.data?.hasValidLicense &&
            isAllDataLoaded && (<div className="flex">
              <div className={classNames(workflow && !workflow.name ? "text-muted" : "")}>
                {workflow && workflow.name ? workflow.name : "untitled"}
              </div>
              {workflow && workflow.team && (<Badge className="mt-1 ml-4" variant="gray">
                  {workflow.team.slug}
                </Badge>)}
              {readOnly && (<Badge className="mt-1 ml-4" variant="gray">
                  {t("readonly")}
                </Badge>)}
            </div>)}>
        <LicenseRequired>
          {!isError ? (<>
              {isAllDataLoaded ? (<>
                  <WorkflowDetailsPage form={form} workflowId={+workflowId} selectedEventTypes={selectedEventTypes} setSelectedEventTypes={setSelectedEventTypes} teamId={workflow ? workflow.teamId || undefined : undefined} isMixedEventType={isMixedEventType} readOnly={readOnly}/>
                </>) : (<SkeletonLoader />)}
