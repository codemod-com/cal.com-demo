...
const WorkflowListItem = (props: ItemProps) => {
    ...
    const [activeEventTypeIds, setActiveEventTypeIds] = useState(workflow.activeOn.map((active) => {
        if (active.eventType) {
    ...
    const activateEventTypeMutation = trpc.viewer.workflows.activateEventType.useMutation({
        onSuccess: async () => {
            ...
            showToast(t("workflow_turned_on_successfully", {
    ...
    return (<div className="border-subtle flex w-full items-center overflow-hidden rounded-md border p-6 px-3 md:p-6">
      ...
      <div className=" grow">
        <div className={classNames("text-emphasis mb-1 w-full truncate text-base font-medium leading-4 md:max-w-max", workflow.name && isActive ? "text-emphasis" : "text-subtle")}>
          {workflow.name
            ? workflow.name
            : "Untitled (" +
        ...
        <div className={classNames(" flex w-fit items-center whitespace-nowrap rounded-sm text-sm leading-4", isActive ? "text-default" : "text-muted")}>
      ...
      {!workflow.readOnly && (<div className="flex-none">
      ...
      <Tooltip content={t(workflow.readOnly && props.isChildrenManagedEventType
            ? "locked_by_admin"
            : isActive
                ? "turn_off"
                : "turn_on") as string}>
        <div className="flex items-center ltr:mr-2 rtl:ml-2">
          {workflow.readOnly && props.isChildrenManagedEventType && (<Lock className="text-subtle h-4 w-4 ltr:mr-2 rtl:ml-2"/>)}
          <Switch checked={isActive} disabled={workflow.readOnly} onCheckedChange={() => {
...
function EventWorkflowsTab(props: Props) {
    ...
    useEffect(() => {
        if (data?.workflows) {
            ...
            const disabledWorkflows = data.workflows.filter((workflow) => !workflows
                .map((workflow) => {
                return workflow.id;
            })
    ...
    return (<LicenseRequired>
      {!isLoading ? (<>
          {isManagedEventType && (<Alert severity="neutral" className="mb-2" title={t("locked_for_members")} message={t("locked_workflows_description")}/>)}
          {data?.workflows && data?.workflows.length > 0 ? (<div>
              <div className="space-y-4">
                {sortedWorkflows.map((workflow) => {
                    return (<WorkflowListItem key={workflow.id} workflow={workflow} eventType={props.eventType} isChildrenManagedEventType/>);
                })}
              </div>
            </div>) : (<div className="pt-2 before:border-0">
              <EmptyScreen Icon={Zap} headline={t("workflows")} description={t("no_workflows_description")} buttonRaw={<Button target="_blank" color="secondary" onClick={() => createMutation.mutate({ teamId: eventType.team?.id })} loading={createMutation.isLoading}>
                    {t("create_workflow")}
                  </Button>}/>
            </div>)}
        </>) : (<SkeletonLoader />)}
