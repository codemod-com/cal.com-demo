...
const registerNewSetter = (registration: {
...
export const useIsEmbed = (embedSsr?: boolean) => {
    ...
    useEffect(() => {
        ...
        if (parent !== window && !_isValidNamespace) {
            log("Looks like you have iframed cal.com but not using Embed Snippet. Directly using an iframe isn't recommended.");
        }
...
const methods = {
    ui: function style(uiConfig: UiConfig) {
        ...
        if (stylesConfig) {
            console.warn("Cal.com Embed: `styles` prop is deprecated. Use `cssVarsPerTheme` instead to achieve the same effect");
        }
