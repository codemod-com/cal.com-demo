...
import { Button, ConfirmationDialogContent, Dialog, DialogClose, DialogContent, Dropdown, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, Form, showToast, Switch, TextAreaField, TextField, SettingsToggle, } from "@calcom/ui";
...
const openModal = (router: NextRouter, option: {
    target?: string;
    action: string;
}) => {
    const query = {
...
function NewFormDialog({ appUrl }: {
    ...
    return (<Dialog name="new-form" clearQueryParamsOnClose={["target", "action"]}>
      <DialogContent className="overflow-y-auto">
        ...
        <Form form={hookForm} handleSubmit={(values) => {
            const formId = uuidv4();
          ...
          <div className="mt-3 space-y-4">
            ...
            <div className="mb-5">
              <TextAreaField id="description" label={t("description")} {...register("description")} data-testid="description" placeholder={t("form_description_placeholder")}/>
            </div>
            {action === "duplicate" && (<Controller name="shouldConnect" render={({ field: { value, onChange } }) => {
                return (<SettingsToggle title={t("keep_me_connected_with_form")} description={t("fields_in_form_duplicated")} checked={value} onCheckedChange={(checked) => {
                        onChange(checked);
                    }}/>);
            }}/>)}
          </div>
...
export const FormActionsDropdown = ({ form, children }: {
    ...
    const { disabled } = form;
    return (<dropdownCtx.Provider value={{ dropdown: true }}>
      <Dropdown>
        <DropdownMenuTrigger data-testid="form-dropdown" asChild>
          <Button type="button" variant="icon" color="secondary" className={classNames("radix-state-open:rounded-r-md", disabled && "opacity-30")} StartIcon={MoreHorizontal}/>
        </DropdownMenuTrigger>
...
function Dialogs({ appUrl, deleteDialogOpen, setDeleteDialogOpen, deleteDialogFormId, }: {
    ...
    return (<div id="form-dialogs">
      <EmbedDialog />
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <ConfirmationDialogContent isLoading={deleteMutation.isLoading} variety="danger" title={t("delete_form")} confirmBtnText={t("delete_form_action")} loadingText={t("delete_form_action")} onConfirm={(e) => {
...
export function FormActionsProvider({ appUrl, children }: {
    ...
    return (<>
      ...
      <Dialogs appUrl={appUrl} deleteDialogFormId={deleteDialogFormId} deleteDialogOpen={deleteDialogOpen} setDeleteDialogOpen={setDeleteDialogOpen}/>
    </>);
}
...
export const FormAction = forwardRef(function FormAction<T extends typeof Button>(props: FormActionProps<T>, forwardedRef: React.ForwardedRef<HTMLAnchorElement | HTMLButtonElement>) {
    const { action: actionName, routingForm, children, as: asFromElement, extraClassNames, ...additionalProps } = props;
    ...
    const { t } = useLocale();
    const router = useRouter();
    const actionData: Record<FormActionType, ButtonProps & {
        ...
        toggle: {
            render: ({ routingForm, label = "", ...restProps }) => {
                ...
                return (<div {...restProps} className={classNames("sm:hover:bg-subtle self-center rounded-md p-2 hover:bg-gray-200", extraClassNames)}>
            <Switch checked={!routingForm.disabled} label={label} onCheckedChange={(checked) => toggle.onAction({ routingForm, checked })} labelOnLeading/>
          </div>);
            },
    ...
    if (!dropdown) {
        return (<Component data-testid={`form-action-${actionName}`} ref={forwardedRef} {...actionProps}>
        {children}
      </Component>);
    }
    return (<DropdownMenuItem>
      <Component ref={forwardedRef} {...actionProps} className={classNames(props.className, "w-full transition-none", props.color === "destructive" && "border-0")}>
        {children}
      </Component>
    </DropdownMenuItem>);
});
