...
import { ConnectButton, darkTheme, getDefaultWallets, lightTheme, RainbowKitProvider, } from "@rainbow-me/rainbowkit";
...
const BalanceCheck: React.FC<RainbowGateProps> = ({ chainId, setToken, tokenAddress }) => {
    ...
    const { data: signedMessage, isLoading: isSignatureLoading, isError: isSignatureError, signMessage, } = useSignMessage({
        message: ETH_MESSAGE,
    });
    ...
    const { data: balanceData, isLoading: isBalanceLoading } = trpc.viewer.eth.balance.useQuery({ address: address || "", tokenAddress, chainId }, {
        enabled: !!address,
    });
    
    // The token may have already been set in the query params, so we can extract it here
    const router = useRouter();
    const ethSignature = searchParams?.get("ethSignature"), routerQuery = searchParams?.get("routerQuery");
    ...
    return (<main className="mx-auto max-w-3xl py-24 px-4">
          ...
          <span className="mb-4 grow md:mb-0">
            ...
            {!isLoading && contractData && contractData.data && (<>
                ...
                {balanceData && balanceData.data && (<>
                    {!balanceData.data.hasBalance && (<div className="mt-2 flex flex-row items-center">
                    ...
                    {balanceData.data.hasBalance && isSignatureLoading && (<div className="mt-2 flex flex-row items-center">
                        <Loader className="h-5 w-5 text-green-600"/>
                        <p className="ml-2 text-green-600">{t("rainbow_sign_message_request")}</p>
                      </div>)}
                  </>)}

                {isSignatureError && (<div className="mt-2 flex flex-row items-center">
                    <AlertTriangle className="h-5 w-5 text-red-600"/>
                    <p className="ml-2 text-red-600">
